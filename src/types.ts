...
export interface PluginOptions extends GenerateSWConfig {
  ...
  fallbacks?: Fallbacks;
  /**
   * Enable additional route caching when navigating between pages with
   * `next/link` on frontend. Check this
   * [example](https://github.com/DuCanhGH/next-pwa/tree/master/examples/cache-on-front-end-nav)
   * for some context on why this is implemented. This improve user experience
   * in some special cases but it also adds some overhead because of additional
   * network calls, you can consider this a tradeoff.
   *
   * @default false
   */
  ...
  scope?: string;
  /**
   * Customize the directory where `next-pwa` looks for a custom workerc
